...
object DeltaMetricsUtils {
   ...
   *
   * @param table
   *   The Delta table to query
   * @return
   *   The operation metrics of the last command.
   */
   ...
   *   The expected metrics the values of which to check.
   * @param operationMetrics
   *   The operation metrics that were collected from Delta log.
   */
  def checkOperationMetrics(
    ...
    val failMessages = expectedMetrics
      .flatMap {
        case (metric, expectedValue) =>
          // Check missing metrics.
          var errMsg = if (!operationMetrics.contains(metric)) {
            Some(s"""The recorded operation metrics does not contain metric: $metric"
          ...
          errMsg = errMsg.orElse {
            if (expectedValue != operationMetrics(metric)) {
              Some(s"""The recorded metric for $metric does not equal the expected value.
   ...
   *
   * @param operationMetrics
   *   The collected operation metrics from the Delta log.
   * @param expectedMetrics
   *   The keys of the expected time metrics. Set to None to check for common time metrics.
   */
  def checkOperationTimeMetrics(
    ...
    for (key <- expectedMetrics) {
      ...
      assert(value >= 0, s"Invalid non-positive value for metric $key: $value")
    }
    ...
    if (expectedMetrics.contains("executionTimeMs")) {
      ...
      val maxTimeMs =
        operationMetrics.filterKeys(k => expectedMetrics.contains(k)).valuesIterator.max
   ...
   *
   * @param deltaLog
   *   The Delta log of the table.
   * @param version
   *   The version of the commit.
   * @return
   *   A map with the expected operation metrics.
   */
  def getOperationMetricsFromCommitActions(deltaLog: DeltaLog, version: Long): Map[String, Long] = {
